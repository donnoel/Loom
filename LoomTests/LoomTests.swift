...
struct AIChatbotStatusClientTests {
    @Test
    func parseAtlassianSnapshotMapsStatusAndIncidents() throws {
        let data = Data(
            """
            {
              "status": {
              ...
              "incidents": [
                {
                  "name": "Elevated latency for chat requests",
        ...
        let snapshot = try AIChatbotStatusClient.parseAtlassianSnapshot(
            from: data,
            serviceID: "chatgpt",
            serviceName: "ChatGPT",
        ...
        #expect(snapshot.state == .degraded)
        #expect(snapshot.summary == "Partial System Outage")
        #expect(snapshot.knownIssues.contains("Elevated latency for chat requests (Investigating)"))
        #expect(snapshot.knownIssues.contains("Maintenance: Database maintenance window"))
    }

    @Test
    func parseAtlassianSnapshotHandlesFeedsWithoutIncidentArrays() throws {
        let data = Data(
            """
            {
              "status": {
        ...
        #expect(snapshot.state == .operational)
        #expect(snapshot.summary == "All Systems Operational")
        #expect(snapshot.knownIssues.isEmpty)
    }

    @Test
    func parseInstatusSnapshotMapsOutageAndKnownIssues() throws {
        let data = Data(
            """
            {
              "page": {
        ...
        let snapshot = try AIChatbotStatusClient.parseInstatusSnapshot(
            from: data,
        ...
        #expect(snapshot.state == .outage)
        #expect(snapshot.summary == "1 known issue.")
        #expect(snapshot.knownIssues == ["Login failures (Identified)"])
    }

    @Test
    func parseStatusPageHTMLSnapshotDetectsOperationalState() {
        let data = Data(
            """
            <html>
              <body>
                <h1>We're fully operational</h1>
        ...
        let snapshot = AIChatbotStatusClient.parseStatusPageHTMLSnapshot(
            from: data,
            serviceID: "poe",
            serviceName: "Poe",
    ...
    @Test
    func parseStatusPageHTMLSnapshotDetectsDegradedStateAndExtractsIssueLines() {
        let data = Data(
            """
            <html>
              <body>
                <p>Investigating elevated error rates for chat requests.</p>
                <p>Some users are seeing degraded performance right now.</p>
              </body>
            </html>
            """.utf8
        )
        ...
        #expect(snapshot.state == .degraded)
        #expect(snapshot.summary == "Status page reports active issues.")
        #expect(snapshot.knownIssues.contains(where: { $0.contains("Investigating") }))
    }

    @Test
    func parseStatusPageHTMLSnapshotIgnoresNextScriptNoiseWhenOperationalSignalsExist() {
        let data = Data(
            """
            <html>
              <body>
                ...
                <p>We’re not aware of any issues affecting our systems.</p>
                <script>
                self.__next_f.push([1,"incident_statuses\":{\"identified\":\"Identified\"},\"children\":[\"$\",\"html\"],\"_next/static/chunks/123.js\""]);
                </script>
              </body>
            </html>
            """.utf8
